...
@Component({
  selector: 'app-menu-restaurant-id',
  templateUrl: './menu-restaurant-id.component.html',
  styleUrl: './menu-restaurant-id.component.css'
})
export class MenuRestaurantIdComponent implements OnInit{
  menuItems: any[] = [];
  ...
  fetchMenuItems(): void {
    const url = `http://localhost:36000/api/Customer/GetMenuByRestaurant?restaurantId=${this.restaurantId}`;
    this.http.get<any[]>(url, this.authHeader).subscribe(response => {
      this.menuItems = response;
      //Modification
      if (response.length > 0) {
        this.restaurantName = response[0].restaurant.restaurantName; // Add this line to get the restaurant name
      }
    }, error => {
  ...
  filterMenuItems(): void {
    if (this.selectedCategory === 'all') {
      ...
      this.filteredMenuItems = this.menuItems.filter(item => item.cuisine.toLowerCase() === this.selectedCategory.toLowerCase() || item.restaurant.restaurantName.toLowerCase() === this.selectedCategory.toLowerCase());
    }
  }
  ...
  searchMenuItems(): void {
    ...
    const url = `http://localhost:36000/api/Customer/GetMenuByRestaurant?restaurantId=${this.restaurantId}&searchQuery=${searchQuery}`;
    this.http.get<any[]>(url, this.authHeader).subscribe(response => {
      this.menuItems = response;
    }, error => {
