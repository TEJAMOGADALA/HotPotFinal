...
namespace HotPot.Services
{
    public class CustomerServices : ICustomerServices
    {
        ...
        public async Task<LoginUserDTO> LogIn(LoginUserDTO loginCustomer)
        {
            ...
            //modification
            //var city = await _cityRepo.GetAsync(loginCustomer.CityName);
            //var cities = await _cityRepo.GetAsync();
            //var cityname = cities.FirstOrDefault(c => c.Name == loginCustomer.CityName);
            ...
            {
                //modify
                //loginCustomer.CityName = cityname.Name;
                if (user.Role == "Customer")
                    loginCustomer.UserId = (_context.Customers.FirstOrDefault(x => x.UserName == user.UserName)).Id;

                if (user.Role == "RestaurantOwner")
                {
                    RestaurantOwner ro = _context.RestaurantOwners.FirstOrDefault(x => x.UserName == user.UserName);
                    loginCustomer.UserId = ro.OwnerId;
                    loginCustomer.RestaurantId = ro.RestaurantId;
                }
                if (user.Role == "DeliveryPartner")
                {
                    DeliveryPartner dp = _context.DeliveryPartners.FirstOrDefault(x => x.UserName == user.UserName);
                    loginCustomer.UserId = dp.PartnerId;
                    loginCustomer.UserName = dp.UserName;
                }
